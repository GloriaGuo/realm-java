...
public class OrderedCollectionChangeTests {
    ...
    private void checkRanges(OrderedCollectionChange.Range[] ranges, long... indexAndLen) {
        ...
        for (int i = 0; i < ranges.length; i++) {
            OrderedCollectionChange.Range range = ranges[i];
    ...
    public void deletion() {
        ...
        results.addChangeListener(new OrderedRealmCollectionChangeListener<RealmResults<AllTypes>>() {
            ...
            public void onChange(RealmResults<AllTypes> collection, OrderedCollectionChange changes) {
                ...
                assertNull(changes.getChangeRanges());
                assertNull(changes.getInsertionRanges());
                assertNull(changes.getChanges());
                assertNull(changes.getInsertions());
    ...
    public void insertion() {
        ...
        results.addChangeListener(new OrderedRealmCollectionChangeListener<RealmResults<AllTypes>>() {
            ...
            public void onChange(RealmResults<AllTypes> collection, OrderedCollectionChange changes) {
                ...
                assertNull(changes.getChangeRanges());
                assertNull(changes.getDeletionRanges());
                assertNull(changes.getChanges());
                assertNull(changes.getDeletions());
                looperThread.testComplete();
            }
        });
